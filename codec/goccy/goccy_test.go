...
var codec jsonrpc2.Codec = goccy.Codec{}
...
func TestCodec_RoundTrip(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			dstGoccy := tt.into()
			if err := codec.Unmarshal(data, dstGoccy); err != nil {
			...
			if diff := gocmp.Diff(dstRef, dstGoccy); diff != "" {
				t.Errorf("round-trip mismatch vs default codec (-default +goccy):\n%s", diff)
			}
		})
	}
}
